...
func (s *QualityService) Check(ctx context.Context, content string) (*QualityResult, []byte, error) {
	prompt := fmt.Sprintf(`你是一个银行生产运维文档审核专家。

请检查下面的运维手册是否适合进入生产知识库。

请从完整性、准确性、可操作性、可验证性、可追溯性评分，总分 100 分。
请输出 JSON，不要输出多余解释。
...
手册内容：
%s`, truncate(content, 12000))
	resp, err := s.llm.Chat(ctx, []client.ChatMessage{{Role: "user", Content: prompt}})
	if err != nil {
		return fallbackQuality(content), mustJSON(fallbackQuality(content)), nil
	}
...
func fallbackQuality(content string) *QualityResult {
	score := 75
	level := "warning"
